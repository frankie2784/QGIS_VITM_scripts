...
def find_next_seq(seq, nodes_to_search):
    ...
    while True:
        ...
        if len(new_nodes) == 1:
            ...
            if len(seq) > 2 and any(node == first_node for node in next_nodes):
                seq.append(seq[0])
            return seq
        else:
            return seq
    ...
    while True:
        ...
        for start_node in found_nodes:
            ...
            for next_node in next_nodes:
                ...
                if seq[-1] == seq[0]:
                ...
                elif seq[-1] == dest_node:
                    if seq not in both_directions(all_seqs['Link']):
                        all_seqs['Link'].append(seq)
                        temp_seq.append(seq)
                elif seq[-1] == orig_node:
                    None           
                elif seq[-1] not in junction_nodes:
                    ...
                    if seq not in both_directions(all_seqs['Link']):
                        all_seqs['Link'].append(seq)
                        temp_seq.append(seq)
                if seq in temp_seq and seq[-1] not in all_nodes:
                    all_nodes.append(seq[-1])
                    new_nodes.append(seq[-1])
    ...
    for path in paths:
        ...
        for i in range(len(flat_path) - 1):
            try:
                ...
                found_seq = [seq for seq in both_directions(all_seqs['Link']) if seq[0] == flat_path[i] and seq[-1] == flat_path[i + 1]][0]
...
def find_shortest_path(orig_node, dest_node, traversed_paths, shortest_number_of_nodes):
    ...
    while True:
        ...
        for nodes in node_tree:
            new_nodes = [seq for seq in both_directions(all_seqs['Link']) if seq[0] == nodes[-1]]
            new_nodes = list(set([seq[-1] for seq in new_nodes if seq not in both_directions(traversed_paths)]))
...
def consolidate_paths(direct_paths, other_branches, orig_node, dest_node, end_node):
    ...
    loops_at_dest = [loop for loop in all_seqs['Loop'] if loop[0] == dest_node]
    paths_beginning_at_dest = [path for path in both_directions(all_seqs['Link']) if path[0] == dest_node]
    ...
    paths_beginning_at_last_node = [path for path in both_directions(all_seqs['Link']) if path[0] == end_node]
    ...
    for path in paths_not_ending_at_dest:
        if first_node(path) == final_node(path):
            ...
            if final_node(path) not in [dest_node, end_node]:
    ...
    if len(paths_ending_at_dest) % 2 == 0 and len(paths_beginning_at_dest) > 0:
...
def find_most_likely_path(orig_node, dest_node, traversed_paths, previous_shortest_path, end_node):

    last_node = previous_shortest_path[-1]
    ...
    for i in range(len(shortest_path)):
        ...
        if i < len(shortest_path) - 1:
            ...
            if current_node not in previous_shortest_path[:-1]:
                print('\nsearching past',current_node)
            else:
                continue
        paths = [seq for seq in both_directions(all_seqs['Link']) if seq[0] == current_node]
        paths = [seq for seq in paths if seq not in both_directions(traversed_paths)]
        print('all paths starting from',current_node,':',paths)
        if len(paths) > 0:
            if i < len(shortest_path) - 1:
                direct_paths = [path for path in paths if path[-1] == next_node]
                traversed_paths += direct_paths
            ...
            if branch:
                ...
                other_branches = find_most_likely_path(current_node, next_node, traversed_paths, shortest_path, end_node)
                print('other branches',current_node,next_node,other_branches)
                traversed_paths += other_branches
            else:
                other_branches = []
            build_tree += consolidate_paths(direct_paths, other_branches, current_node, next_node, end_node)
...
def assert_dest_node(path, dest_node):
    ...
    if end_node != dest_node:
        ...
        for i in range(len(shortest_path) - 1):
            current_node = shortest_path[i]
            next_node = shortest_path[i + 1]
            paths = [seq for seq in both_directions(all_seqs['Link']) if (seq[0] == current_node and seq[-1] == next_node)]
...
def generate_node_sequence(junction_nodes, orig_node, dest_node):
    ...
    traversed_paths = []
    previous_shortest_path = find_shortest_path(orig_node, dest_node, traversed_paths, True)
    end_node = dest_node
    build_tree = find_most_likely_path(orig_node, dest_node, traversed_paths, previous_shortest_path, end_node)
    ...
    for loop in all_seqs['Loop']:
        ...
        for i in reversed(range(len(build_tree))):
            if loop[0] == build_tree[i][0]:
                build_tree.insert(i, loop)
                break
            elif loop[0] == build_tree[i][-1]:
